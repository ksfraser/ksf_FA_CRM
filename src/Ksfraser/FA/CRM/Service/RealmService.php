...
/**
 * RealmService — DDL caching + hooks for realm reference data.
 *
...
class RealmService
{
    ...
    public function __construct(?RealmRepository $repo = null)
    {
        $this->repo = $repo ?? new RealmRepository();
    }
    ...
    public function getHtmlOptions(
        ...
        $options = $this->getOrBuildOptions($dataKey, function () use ($activeOnly, $blankLabel, $formatString) {
            ...
            foreach ($entities as $entity) {
                $text = str_replace(
                    ['{name}', '{id}', '{default_stage}'],
                    [$entity->getName(), (string)$entity->getId(), $entity->getDefaultStage()],
                    $formatString
                );
    ...
    public function getRealmSelect(
        string $name = 'realm_id',
        bool $activeOnly = true,
        string $blankLabel = '',
        string $formatString = '{name}',
    ...
    public function hookGetRealmDDL(array &$data, $opts = null): array
    {
        return $this->getDdl(
    ...
    public function hookGetRealmHtmlOptions(array &$data, $opts = null): array
    {
